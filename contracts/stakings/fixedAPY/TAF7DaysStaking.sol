...
contract TAF7DaysStaking {
    ...
    modifier onlyAfter7Days {
      require(BokkyPooBahsDateTimeLibrary.diffDays(timestamp[msg.sender], block.timestamp ) >= 14);
      _;
   }
    ...
    function timings() public view returns (uint256, uint256){
        return (timestamp[msg.sender], timestamp[msg.sender] + 14 days);
    }
}